function shadeVector(C,varargin)
%% function shadeVector(C,[color])
% =======================
...
% periods. If a seconfd argument is passed, the color is defined.
% ======================
% NEB 2016_06
%%
if length(varargin)==1
    fill_color = varargin{1};
else
    fill_color = 'k';
end
...
if ~isvector(C)% if the input is already converted from a binary vector to onsets, then skip conversion
    ...
    for ii = 1:length(cStart)
        fill([cStart(ii) cStart(ii) cEnd(ii) cEnd(ii)],[ax.YLim(1) ax.YLim(2) ax.YLim(2) ax.YLim(1)],fill_color,'facealpha',.2,'edgecolor','none')
    end
end